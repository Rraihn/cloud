...
func (i InvoiceRepositoryImpl) SaveInvoice(ctx context.Context, tx *sql.Tx, invoice domain.Invoice) domain.Invoice {
	SQL := "insert into discount(invoice_date, tax, price, invoice_number, total)"
	result, err := tx.ExecContext(ctx, SQL, invoice.Invoice_Date, invoice.Tax, invoice.Price, invoice.Invoice_number, invoice.Total)
...
func (i InvoiceRepositoryImpl) UpdateInvoice(ctx context.Context, tx *sql.Tx, invoice domain.Invoice) domain.Invoice {
	SQl := "update invoice set tax = ?, price = ?, total = ? where invoice_id"
	_, err := tx.ExecContext(ctx, SQl, invoice.Tax, invoice.Price, invoice.Total, invoice.Invoice_id)
	helper.PanicIfError(err)
...
func (i InvoiceRepositoryImpl) FindInvoiceById(ctx context.Context, tx *sql.Tx, invoiceId int) (domain.Invoice, error) {
	SQL := "select invoice_id, invoice_date, tax, price, invoice_number, total from invoice where invoice_id = ?"
	...
	if rows.Next() {
		err := rows.Scan(&invoices.Invoice_id, &invoices.Invoice_Date, &invoices.Tax, &invoices.Price, &invoices.Invoice_number, &invoices.Total)
		helper.PanicIfError(err)
		defer rows.Close()
		return invoices, nil
	} else {
...
func (i InvoiceRepositoryImpl) FindAllInvoice(ctx context.Context, tx *sql.Tx) []domain.Invoice {
	SQL := "select invoice_id, invoice_date, tax, price, invoice_number, total from invoice"
